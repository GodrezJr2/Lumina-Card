    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 antialiased overflow-x-hidden" style={{ fontFamily: "'Noto Serif', 'Georgia', serif" }}>
      ...
      <div className="relative flex flex-col items-center justify-center min-h-screen w-full px-4 overflow-hidden">
          ...
          <div
            ...
            style={{
              backgroundImage: gallery[0]
                ? `linear-gradient(rgba(15,23,42,0.65) 0%, rgba(15,23,42,0.82) 100%), url('${gallery[0]}')`
        ...
        <motion.div
          ...
          className="absolute inset-6 border border-yellow-400/30 pointer-events-none rounded z-10"
        />
        <motion.div
          initial={{ opacity: 0, scale: 1.1 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1.4, ease: [0.22, 1, 0.36, 1] }}
          className="absolute inset-8 border border-yellow-400/15 pointer-events-none rounded z-10"
        />
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="relative z-20 flex flex-col items-center text-center max-w-3xl mx-auto space-y-8"
        >
          ...
          <h1 className="text-5xl md:text-7xl font-black leading-tight text-white drop-shadow-lg">
            <SplitText text={coupleNames || eventName} staggerDelay={0.09} y={80} delay={0.3} />
          </h1>
      ...
      {/* ── MARQUEE DIVIDER ── */}
      <Marquee
        items={["Royal Wedding", "Pernikahan Mewah", "Keluarga Bahagia", "Cinta Abadi"]}
        className="py-3 bg-yellow-400/10 text-yellow-400/80 text-xs font-bold tracking-widest uppercase border-y border-yellow-400/20"
        speed={20}
        separator="♛"
      />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 space-y-16">
        <div>
          <FloatIn className="text-center mb-10">
            ...
            <h2 className="text-yellow-400 text-3xl font-bold mt-6">Cerita Kami</h2>
          </FloatIn>

          <FloatIn className="grid grid-cols-1 md:grid-cols-3 gap-6" staggerDelay={0.12}>
            {[
              ...
              <div key={i} className="flex flex-col items-center text-center gap-4 rounded-xl border border-yellow-400/20 bg-slate-800 p-6 hover:border-yellow-400/50 transition-colors">
        ...
        {gallery.filter(Boolean).length > 0 && (
          <div>
            <FloatIn className="text-center mb-8">
              <h2 className="text-yellow-400 text-3xl font-bold">Galeri Foto</h2>
            </FloatIn>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 rounded-xl overflow-hidden">
              {gallery.filter(Boolean).map((url, i) => (
                ...
                >
                  ...
                  <img src={url} alt={`Foto ${i + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500 absolute inset-0" />
        ...
        <FloatIn className="flex flex-col items-center gap-6 py-8">
